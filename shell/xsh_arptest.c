...
shellcmd xsh_arptest(int nargs, char *args[])
{
	...
	// find an empty slot in arpcache and put a dummy entry in it	
	// loop over whole arpcache to find an entry whose state is AR_FREE
	for(int32 slot = 0; slot < ARP_SIZ; slot++)
	{
		if(arpcache[slot].arstate == AR_FREE)
		{
			// found a free slot in arpcache, now put a dummy entry in it
			arpcache[slot].arstate = AR_RESOLVED;
			arpcache[slot].arpaddr = 10.10;
